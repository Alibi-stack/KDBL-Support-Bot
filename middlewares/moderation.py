...
class ModerationMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if warnings <= MAX_WARNINGS:
            ...
            await event.answer(
                pick(
                    ...
                    "матерных слов. Если вы продолжите общаться в таком формате, "
                    "бот отправит отчет о чате в Департамент управления "
                    "человеческими ресурсами и СБ (Службу безопасности). "
                    "После предупреждений будет мут на 30 минут.",
                    f"Ескерту {warnings}/{MAX_WARNINGS}: дөрекі сөз қолданбаңыз. "
                    "Осы форматта сөйлесуді жалғастырсаңыз, бот чат есебін "
                    "Адам ресурстарын басқару департаментіне және ҚҚ-ға "
                    "(Қауіпсіздік қызметіне) жібереді. Ескертулерден кейін "
                    "30 минутқа шектеу қойылады.",
                )
            )
            return None
        ...
        await event.answer(
            pick(
                language,
                "Вы получили мут на 30 минут за матерные слова. Отчет о чате "
                "может быть передан в Департамент управления человеческими "
                "ресурсами и СБ (Службу безопасности).",
