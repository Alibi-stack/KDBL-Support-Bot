...
Проявляй инициативу: предлагай следующий разумный шаг, проверку или короткий план действий, даже если пользователь описал проблему очень кратко. Тон уверенный и поддерживающий: ты не просто переадресуешь к оператору, а сначала стараешься помочь сам. Можно использовать смысл: "Я постараюсь помочь здесь и сейчас".
...
Отвечай на вопросы только на основе информации из раздела "КОНТЕКСТ" (фрагменты базы знаний). Если ответа в контексте нет или он не покрывает вопрос, прямо скажи об этом, но не начинай с переадресации: сначала предложи безопасные общие проверки или уточнение. Никогда не придумывай процедуры, номера телефонов, сроки или ссылки. Не называй имена, внутренние номера или контакты сотрудников в AI-ответе; для контактов пользователь может открыть "Справочник номеров".
...
async def _get_local_it_fallback(
    ...
    if any(word in combined for word in ("симбейс", "simbase")):
        return (
            "Если Simbase не работает, уточните, что именно не получается: вход, доступ, "
            "выпуск/замена, КПД, документ или конкретная ошибка. Попробуйте перезапустить "
            "приложение и проверить подключение к сети. Если есть скриншот или текст ошибки, "
            "отправьте его оператору через обращение."
        )

    if any(word in combined for word in ("лотус", "lotus")):
        return (
            "Если Lotus не работает, попробуйте перезапустить приложение или проверить "
            "соединение с сервером. Уточните, что именно не получается: вход, доступ, поиск "
            "документа, регистрация, подписание, отправка или конкретная ошибка. Если проблема "
            "сохраняется, отправьте скриншот или текст ошибки оператору через обращение."
        )

    if any(word in combined for word in ("метадок", "metadoc")):
        return (
            "Если Metadoc не работает, попробуйте обновить страницу, перезапустить приложение "
            "или браузер и проверить подключение к сети. Уточните, что именно не получается: "
            "вход, доступ, поиск документа, подписание, отправка или конкретная ошибка. Если "
            "проблема сохраняется, отправьте скриншот или текст ошибки оператору через обращение."
        )
    ...
    if any(
        ...
        return (
            ...
            "перезапустите принтер. Если есть код ошибки или скриншот, отправьте его "
            "оператору через обращение."
        )
